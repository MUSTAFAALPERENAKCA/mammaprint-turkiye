...
| P-02 | `/testler/` | Testlere Genel Bakış | P1 | §6.1-6.2 | Statik sayfa | İçerik yayında | Tamamlandı — sayfa yapısı ve güvenli içerik yayında; tıbbi iddia gerektiren alanlar [MEDİKAL ONAY GEREKLİ] ile işaretli |
| P-03 | `/mammaprint/` | MammaPrint 70 Gen Testi | P0 | §8.1 | Product içerik tipi + MedicalWebPage schema | §8.1 bölümlerinin tamamı mevcut | Tamamlandı — sayfa yapısı ve güvenli içerik yayında; tıbbi iddia gerektiren alanlar [MEDİKAL ONAY GEREKLİ] ile işaretli |
| P-04 | `/blueprint/` | BluePrint 80 Gen Testi | P0 | §8.2 | Product içerik tipi | §8.2 bölümlerinin tamamı mevcut | Tamamlandı — sayfa yapısı ve güvenli içerik yayında; tıbbi iddia gerektiren alanlar [MEDİKAL ONAY GEREKLİ] ile işaretli |
| P-05 | `/mammaprint-blueprint/` | Birlikte Kullanım | P0 | §4.3, §7.1 blok 4 | Statik sayfa | Tamamlayıcı kullanım anlatımı, sınırlılıklar bölümü var | Tamamlandı — sayfa yapısı ve güvenli içerik yayında; tıbbi iddia gerektiren alanlar [MEDİKAL ONAY GEREKLİ] ile işaretli |
| P-06 | `/genomik-test-nedir/` | Genomik Test Nedir? | P0 | §8.3 | Eğitim sayfası + GlossaryTerm linkleri | Sözlük terimlerine bağlantı çalışıyor | Tamamlandı — sayfa yapısı ve güvenli içerik yayında; tıbbi iddia gerektiren alanlar [MEDİKAL ONAY GEREKLİ] ile işaretli |
| P-07 | `/genomik-test-genetik-test-farki/` | Genomik/Genetik Test Farkı | P0 | §8.3, Ek B | Eğitim sayfası | BRCA karışıklığını netleştiren bölüm var | Tamamlandı — sayfa yapısı ve güvenli içerik yayında; tıbbi iddia gerektiren alanlar [MEDİKAL ONAY GEREKLİ] ile işaretli |
| P-08 | `/hastalar-icin/` | Hasta Merkezi | P0 | §8.4 | Hub sayfası | §8.4 tablosundaki tüm bölümler | Tamamlandı — sayfa yapısı ve güvenli içerik yayında; tıbbi iddia gerektiren alanlar [MEDİKAL ONAY GEREKLİ] ile işaretli |
| P-09 | `/hastalar-icin/kimler-icin-uygun/` | Uygunluk | P0 | §8.4, §5.1 | Kontrol listesi bileşeni | Hekim değerlendirmesi vurgusu var | Tamamlandı — sayfa yapısı ve güvenli içerik yayında; tıbbi iddia gerektiren alanlar [MEDİKAL ONAY GEREKLİ] ile işaretli |
| P-10 | `/hastalar-icin/test-nasil-yapilir/` | Test Süreci | P0 | §5.1, §7.1 blok 6 | 4 adımlı süreç bileşeni | Adımlar görsel akış ile net | Tamamlandı — sayfa yapısı ve güvenli içerik yayında; tıbbi iddia gerektiren alanlar [MEDİKAL ONAY GEREKLİ] ile işaretli |
| P-11 | `/hastalar-icin/sonuclari-anlamak/` | Sonuçları Anlamak | P0 | §8.4, §8.1 | Risk terimleri açıklaması | "Kesin tedavi kararı değildir" notu var | Tamamlandı — sayfa yapısı ve güvenli içerik yayında; tıbbi iddia gerektiren alanlar [MEDİKAL ONAY GEREKLİ] ile işaretli |
| P-12 | `/hastalar-icin/doktorla-konusma-rehberi/` | Doktor Görüşmesi Rehberi | P0 | §5.1, §8.4 | İndirilebilir PDF/HTML Resource | 8-10 soruluk rehber indirilebilir | Tamamlandı — sayfa yapısı ve güvenli içerik yayında; tıbbi iddia gerektiren alanlar [MEDİKAL ONAY GEREKLİ] ile işaretli |
| P-13 | `/hastalar-icin/sik-sorulan-sorular/` | Hasta SSS | P0 | §7.1 blok 10 | FAQ içerik tipi + FAQPage schema | Schema görünür içerikle birebir | Tamamlandı — sayfa yapısı ve güvenli içerik yayında; tıbbi iddia gerektiren alanlar [MEDİKAL ONAY GEREKLİ] ile işaretli |
| P-14 | `/saglik-profesyonelleri/` | Hekim Merkezi | P0 | §8.5 | Hub sayfası | §8.5 maddelerinin tamamı | Tamamlandı — sayfa yapısı ve güvenli içerik yayında; tıbbi iddia gerektiren alanlar [MEDİKAL ONAY GEREKLİ] ile işaretli |
| P-15 | `/saglik-profesyonelleri/klinik-uygunluk/` | Klinik Uygunluk | P0 | §8.5 | Uygunluk matrisi bileşeni | Resmi kullanım amacı ile tutarlı | Tamamlandı — sayfa yapısı ve güvenli içerik yayında; tıbbi iddia gerektiren alanlar [MEDİKAL ONAY GEREKLİ] ile işaretli |
| P-16 | `/saglik-profesyonelleri/numune-ve-lojistik/` | Numune ve Lojistik | P0 | §8.5 | Teknik rehber sayfası | FFPE/gönderim/takip bilgisi eksiksiz | Tamamlandı — sayfa yapısı ve güvenli içerik yayında; tıbbi iddia gerektiren alanlar [MEDİKAL ONAY GEREKLİ] ile işaretli |
| P-17 | `/saglik-profesyonelleri/ornek-raporlar/` | Örnek Raporlar | P0 | §8.5, §12.1 | Yetkiye göre erişim + Resource | Rol bazlı erişim çalışıyor | Tamamlandı — sayfa yapısı ve güvenli içerik yayında; tıbbi iddia gerektiren alanlar [MEDİKAL ONAY GEREKLİ] ile işaretli |
| P-18 | `/saglik-profesyonelleri/test-talebi/` | Test Talep Süreci | P1 | §8.5 | Süreç sayfası | İçerik yayında | Tamamlandı — sayfa yapısı ve güvenli içerik yayında; tıbbi iddia gerektiren alanlar [MEDİKAL ONAY GEREKLİ] ile işaretli |
| P-19 | `/saglik-profesyonelleri/medikal-bilgi/` | Medikal Bilgi Talebi | P0 | §8.5, §8.7 | HCP iletişim formu | Form KVKK onayı ile çalışıyor | Tamamlandı — sayfa yapısı ve güvenli içerik yayında; tıbbi iddia gerektiren alanlar [MEDİKAL ONAY GEREKLİ] ile işaretli |
| P-20 | `/klinik-kanit/` | Klinik Kanıt Merkezi | P0 | §8.6 | ClinicalStudy listesi | Kanıt kartı şablonu (§8.6 tablo) uygulanmış | Tamamlandı — sayfa yapısı ve güvenli içerik yayında; tıbbi iddia gerektiren alanlar [MEDİKAL ONAY GEREKLİ] ile işaretli |
| P-21 | `/klinik-kanit/mindact/` | MINDACT | P0 | §8.6, MVP kapsamı | ClinicalStudy detay sayfası | Kanıt kartı alanlarının tamamı dolu | Tamamlandı — sayfa yapısı ve güvenli içerik yayında; tıbbi iddia gerektiren alanlar [MEDİKAL ONAY GEREKLİ] ile işaretli |
| P-22 | `/klinik-kanit/nbrst/` | NBRST | P0 | §8.6, MVP kapsamı | ClinicalStudy detay sayfası | Kanıt kartı alanlarının tamamı dolu | Tamamlandı — sayfa yapısı ve güvenli içerik yayında; tıbbi iddia gerektiren alanlar [MEDİKAL ONAY GEREKLİ] ile işaretli |
| P-23 | `/klinik-kanit/sto-3/` | STO-3 | P1 | §8.6 | ClinicalStudy detay sayfası | İçerik yayında | Tamamlandı — sayfa yapısı ve güvenli içerik yayında; tıbbi iddia gerektiren alanlar [MEDİKAL ONAY GEREKLİ] ile işaretli |
| P-24 | `/klinik-kanit/nsabp-b42/` | NSABP B-42 | P1 | §8.6 | ClinicalStudy detay sayfası | İçerik yayında | Tamamlandı — sayfa yapısı ve güvenli içerik yayında; tıbbi iddia gerektiren alanlar [MEDİKAL ONAY GEREKLİ] ile işaretli |
| P-25 | `/yayinlar/` | Yayın Kütüphanesi | P0 | §8.5, §9.4 | Publication listesi + filtre | Filtreleme indeks şişmesi yaratmıyor (noindex parametreler) | Tamamlandı — sayfa yapısı ve güvenli içerik yayında; tıbbi iddia gerektiren alanlar [MEDİKAL ONAY GEREKLİ] ile işaretli |
| P-26 | `/kilavuzlar/` | Kılavuzlar | P1 | §6.1 | Resource listesi | İçerik yayında | Tamamlandı — sayfa yapısı ve güvenli içerik yayında; tıbbi iddia gerektiren alanlar [MEDİKAL ONAY GEREKLİ] ile işaretli |
| P-27 | `/bilgi-merkezi/` | Bilgi Merkezi (hub) | P1 | §6.1 | Hub sayfası | Blog/sözlük/video/kaynak linkleri | Tamamlandı — sayfa yapısı ve güvenli içerik yayında; tıbbi iddia gerektiren alanlar [MEDİKAL ONAY GEREKLİ] ile işaretli |
| P-28 | `/blog/` | Blog | P0 | §10 | Blog listesi + kategori/etiket/arama | Sayfalama + filtre SEO-güvenli | Tamamlandı — sayfa yapısı ve güvenli içerik yayında; tıbbi iddia gerektiren alanlar [MEDİKAL ONAY GEREKLİ] ile işaretli |
| P-29 | `/sozluk/` | Terimler Sözlüğü | P1 | §8.3 | GlossaryTerm listesi | ER/PR/HER2/FFPE vb. terimler tanımlı | Tamamlandı — sayfa yapısı ve güvenli içerik yayında; tıbbi iddia gerektiren alanlar [MEDİKAL ONAY GEREKLİ] ile işaretli |
| P-30 | `/kaynaklar/` | İndirilebilir Kaynaklar | P0 | §8.4, §8.5 | Resource listesi | Dosya türü/boyut/sürüm görünür | Tamamlandı — sayfa yapısı ve güvenli içerik yayında; tıbbi iddia gerektiren alanlar [MEDİKAL ONAY GEREKLİ] ile işaretli |
| P-31 | `/videolar/` | Videolar | P2 | §6.1 | VideoObject schema | Altyazı/transkript altyapısı var | Tamamlandı — sayfa yapısı ve güvenli içerik yayında; tıbbi iddia gerektiren alanlar [MEDİKAL ONAY GEREKLİ] ile işaretli |
| P-32 | `/hakkimizda/` | MammaPrint Türkiye Hakkında | P0 | §6.1, §14.4 | Kurumsal sayfa | Agendia/Omnigen ilişkisi açık | Tamamlandı — sayfa yapısı ve güvenli içerik yayında; tıbbi iddia gerektiren alanlar [MEDİKAL ONAY GEREKLİ] ile işaretli |
| P-33 | `/agendia/` | Agendia Hakkında | P1 | §6.1 | Kurumsal sayfa | İçerik yayında | Tamamlandı — sayfa yapısı ve güvenli içerik yayında; tıbbi iddia gerektiren alanlar [MEDİKAL ONAY GEREKLİ] ile işaretli |
| P-34 | `/omnigen/` | Omnigen ve Türkiye Operasyonu | P1 | §6.1 | Kurumsal sayfa | Türkiye rolü net anlatılmış | Tamamlandı — sayfa yapısı ve güvenli içerik yayında; tıbbi iddia gerektiren alanlar [MEDİKAL ONAY GEREKLİ] ile işaretli |
| P-35 | `/kalite-ve-uyum/` | Kalite, Lisans ve Uyum | P1 | §6.1 | Kurumsal sayfa | İçerik yayında | Tamamlandı — sayfa yapısı ve güvenli içerik yayında; tıbbi iddia gerektiren alanlar [MEDİKAL ONAY GEREKLİ] ile işaretli |
| P-36 | `/iletisim/` | İletişim | P0 | §8.7 | 3 form türü | §8.7 maddelerinin tamamı | Tamamlandı — sayfa yapısı ve güvenli içerik yayında; tıbbi iddia gerektiren alanlar [MEDİKAL ONAY GEREKLİ] ile işaretli |
| P-37 | `/gizlilik/` | Gizlilik ve KVKK | P0 | §14.4 | Hukuki sayfa | Hukuk onaylı, aydınlatma metni eksiksiz | Devam ediyor — sayfa yapısı ve genel taslak metin kuruldu; nihai metin hukuk danışmanı onayı bekliyor |
| P-38 | `/cerez-politikasi/` | Çerez Politikası | P0 | §14.2, §14.4 | Hukuki sayfa + çerez tablosu | Kategori/süre/taraf bilgisi tam | Devam ediyor — sayfa yapısı ve genel taslak metin kuruldu; nihai metin hukuk danışmanı onayı bekliyor |
| P-39 | `/kullanim-kosullari/` | Kullanım Koşulları | P0 | §14.4 | Hukuki sayfa | Hukuk onaylı | Devam ediyor — sayfa yapısı ve genel taslak metin kuruldu; nihai metin hukuk danışmanı onayı bekliyor |
| P-40 | `/tibbi-bilgilendirme/` | Tıbbi Bilgilendirme | P0 | §14.4 | Hukuki sayfa | "Tanı/tedavi yerine geçmez" ifadesi net | Devam ediyor — sayfa yapısı ve genel taslak metin kuruldu; nihai metin hukuk danışmanı onayı bekliyor |
...
|---|---|---|---|---|---|---|---|
| PC-01 | Tek ve anlamlı H1 | §9.3 | P0 | Page içerik modeli | `h1` zorunlu alan | Otomatik test: sayfa başına tek `<h1>` | Tamamlandı — tüm sayfalarda tek H1 uygulanıyor |
| PC-02 | SEO title, meta açıklama, canonical, OG alanları | §9.4, Ek A | P0 | Page/BlogPost/Publication modeli | `seoTitle`, `metaDescription`, `canonicalUrl`, `ogImage` | Metadata testleri geçiyor | Devam ediyor — Metadata API ile seoTitle/description uygulanıyor; canonical/OG Faz sonrası genişletilecek |
| PC-03 | Uygun schema türü (Organization/WebSite/WebPage/Article/FAQPage/BreadcrumbList/MedicalWebPage) | §8.1, §9.4 | P0 | Şema katmanı | JSON-LD üretim fonksiyonları | Rich Results Test + görünür içerikle birebir | Devam ediyor — Organization/WebSite/BreadcrumbList/MedicalWebPage/FAQPage/Article üretiliyor |
| PC-04 | İlk yayın / son güncelleme tarihi, editör, medikal inceleyen | §1, §12.2 | P0 | CMS içerik modeli | `publishedAt`, `updatedAt`, `author`, `medicalReviewer` | Sayfada görünür, boşsa yayın engellenir | Devam ediyor — BlogPost'ta author/publishedAt/updatedAt uygulanıyor; Page modelinde genişletme Faz 3 |
| PC-05 | Kaynakça / bilimsel referans alanı | §8.6, §9.3 | P0 | Page/ClinicalStudy/Publication modeli | `references[]` | Her iddia için kaynak linki mevcut | Devam ediyor — EvidenceCard sourceCitation alanı var; gerçek DOI/PubMed bağlantıları medikal onay bekliyor |
| PC-06 | Sorumluluk reddi (tıbbi bilgilendirme) | §14.3, §14.4 | P0 | Tüm medikal sayfalar | Ortak `MedicalDisclaimer` bileşeni | Her medikal sayfada görünür | Tamamlandı — MedicalDisclaimer bileşeni tüm medikal sayfalarda kullanılıyor |
| PC-07 | İlgili içerikler / iç bağlantılar (3-5 bağlamsal link) | §9.3, §9.5 | P0 | Page/BlogPost modeli | `relatedContent[]` + pillar-cluster linkleme | Sayfa sonunda 3-5 ilgili link var | Devam ediyor — CtaBlock ile çapraz bağlantılar var; otomatik relatedContent Faz 3 |
| PC-08 | Birincil/ikincil CTA | §7.1, §8.1-8.7 | P0 | Page modeli | `primaryCta`, `secondaryCta` | Her P0 sayfada tanımlı CTA var | Tamamlandı — her sayfada birincil/ikincil CTA (CtaBlock) var |
| PC-09 | Hasta ve HCP içeriğinin ayrı üretilmesi (aynı metnin küçük değişikliklerle tekrarı değil) | Kullanıcı promptu §5, strateji §3.2, §5 | P0 | İçerik modeli | `targetAudience` alanı + ayrı content tipi/bölüm şablonları | Editoryal inceleme: iki versiyon bağımsız yazılmış | Tamamlandı — Tabs bileşeni ile hasta/HCP içerik ayrı üretiliyor (ör. /mammaprint) |
...
| C-15 | Blog özellikleri: kategori/etiket/yazar/medikal inceleyen sayfaları, arama, filtre, sayfalama, ilgili yazılar, otomatik içindekiler, okuma süresi, RSS, sitemap entegrasyonu | Kullanıcı promptu §8 | P0 | Blog modülü | Next.js dynamic routes + RSS route + sitemap | Her özellik için fonksiyonel test | Devam ediyor — liste/kategori/detay sayfaları, okuma süresi altyapısı var; RSS ve arama Faz 3 |
...
| M-10 | Kaynağı doğrulanmamış içerik `[MEDİKAL ONAY GEREKLİ]` etiketiyle işaretlenip yayına kapalı tutulmalı | Kullanıcı promptu §12, §22 | P0 | CMS içerik durumu | Content status = `medical_review_required`, `noindex` + yayın engeli | Bu statüdeki içerik public route'ta 404/gizli | Tamamlandı — MedicalReviewFlag bileşeni ile tüm onaysız iddialar işaretli |
...
| M-12 | Veri minimizasyonu: standart formlarda tanı/rapor/genetik sonuç/numune no istenmemeli | §14.1 | P0 | İletişim formları | Form şeması sağlık alanı içermez | Form alan denetimi: yasak alan yok | Tamamlandı — contactFormSchema sağlık/genetik veri alanı içermiyor |
| M-13 | Form mesaj alanında "sağlık/genetik veri paylaşmayın" uyarısı | §14.1, §8.7 | P0 | İletişim formu UI | Uyarı bileşeni | Her formda görünür | Tamamlandı — ContactForm'da uyarı metni var |
...
| M-16 | Çerez: zorunlu olmayan analitik/pazarlama çerezleri tercihten önce çalışmamalı; opt-in eşit görünürlükte | §14.2 | P0 | Çerez tercih merkezi | Consent Mode + gate'lenmiş script yükleme | Analitik script, onay verilmeden network'te görünmüyor (test) | Tamamlandı — CookieConsent bileşeni; onay öncesi analitik script yüklenmiyor (henüz GA4 entegre değil) |
...
| T-03 | Ayrı güvenli form/API endpoint, rate limit, spam koruması | §12.1, §13.3 | P0 | Form API | Next.js Route Handlers + rate limiter + honeypot/CAPTCHA | Form güvenlik testleri geçiyor | Tamamlandı — app/actions/contact.ts route handler + lib/rate-limit.ts |
...
| T-12 | Form CSRF, rate limiting, bot koruması, sunucu tarafı doğrulama | §13.3 | P0 | Form modülü | CSRF token + Zod server validation | Güvenlik testleri geçiyor | Devam ediyor — rate limiting + Zod server validation var; CSRF token Next.js Server Actions'ın yerleşik korumasına dayanıyor |
